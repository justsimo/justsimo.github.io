...
(function (window, document, $) {
  ...
  function init() {
    /* Blocca scroll durante il loader via JS (non CSS) per evitare
       layout shift: overflow:hidden via CSS sul body altera svh/height calc */
    document.body.style.overflowY = 'hidden';

    /* Scroll reset preventivo — evita browser scroll restoration */
    ...
    if (!loadingPage || !countEl || !progressBar) {
      ...
      window.addEventListener('load', function () {
        setTimeout(function () {
          /* Rimuovi overflow:hidden PRIMA del fade — il layout si
             stabilizza mentre il loader è ancora visibile (opaco) */
          document.body.style.overflowY = '';
          /* Breve pausa per permettere al browser di completare
             il reflow prima che il loader diventi trasparente */
          requestAnimationFrame(function() {
            requestAnimationFrame(function() {
              loadingPage.style.transition = 'opacity 350ms ease';
    ...
    if (window.AOS && typeof window.AOS.init === 'function') {
      ...
      /* Recupera elementi saltati su scroll veloce */
      let _aosT;
      window.addEventListener('scroll', function(){
        clearTimeout(_aosT);
        _aosT = setTimeout(function(){ window.AOS.refresh(); }, 200);
      }, { passive: true });
    }
    ...
    // Solidifica leggermente la navbar quando si scrolla
    if (nav) {
    ...
    const onAnchorClick = (e, anchor) => {
      ...
      // Chiudi il menu (se aperto) e scrolla in parallelo — più affidabile su mobile.
      // Non aspettiamo hidden.bs.collapse (inaffidabile su alcuni browser mobile).
      ...
      // Un rAF dà al browser il tempo di processare overflow='' prima dello scroll
    ...
    /* ---------------- HARD SKILLS: mostra le prime N, poi "+N altre" ---- */
    (function () {
      const VISIBLE = 6;
      document.querySelectorAll('.project-card .subtitle-blocks').forEach((list) => {
        const items = Array.from(list.querySelectorAll(':scope > li'));
        if (items.length <= 8) return;            // liste corte: mostra tutto
        const extra = items.slice(VISIBLE);
        const setHidden = (h) => extra.forEach(li => { li.style.display = h ? 'none' : ''; });
        setHidden(true);

        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'skills-more';
        btn.setAttribute('aria-expanded', 'false');
        const n = extra.length;
        btn.textContent = '+' + n + ' altre';
        btn.addEventListener('click', () => {
          const expand = btn.getAttribute('aria-expanded') !== 'true';
          setHidden(!expand);
          btn.setAttribute('aria-expanded', String(expand));
          btn.textContent = expand ? 'mostra meno' : ('+' + n + ' altre');
        });
